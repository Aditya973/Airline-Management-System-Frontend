...
function Flight(props) {
    ...
    async function fetchData(){
        const res1 = await fetch(`http://localhost:3010/api/v1/airport/${flightData.departureAirportId}`);
        const res2 = await fetch(`http://localhost:3010/api/v1/airport/${flightData.arrivalAirportId}`);
  ...
  return (
    <div>
        <Card title = {<h1 style={{textAlign:'center'}}>{departureAirport} to {arrivalAirport}</h1>} className='flightCard'>
            ...
            <EditFlight flightId = {flightData.id}/>  
        </Card>
    </div>
  )
}
